...
const VtdForm: React.FC<VtdFormProps> = ({ table, vtdId, tableType }) => {
  ...
  useLayoutEffect(() => {
    if (!table.columns.find(({ value }) => value === REPAIR_COLUMN_NAME)) {
      const typeNumberIndex = table.columns.findIndex(({ value }) => value === REQUIRED_COLUMNS_NAMES.typeNumberVtd) + 1;

      dispatch(addColumn({ vtdId, tableType, name: REPAIR_COLUMN_NAME, index: typeNumberIndex }));
    }
