...
import {
  ...
  createReport,
  createVtd,
} from '../../../vtdManager/helpers/serviceManager';
...
const AddVtdButton: FC<ManageVtdButtonsProps> = ({ vtdId, formRef, selectValues }) => {
  ...
  const addVtdOnClick = async (): Promise<void> => {
    ...
    if (startKm && vtdId) {
      const reportFiles = (formData.getAll(FILE_INPUTS.report.name) as File[]).filter(
        ({ name }) => name !== '',
      );
      ...
      if (joiningFile.name !== '') {
        const prevLevelVtdTree = getVtdTreeChildren({
          vtdTree,
          selectValues,
          selectIndex: VTD_TREE_LEVELS_KEYS.length - 1,
        });
        const prevLevelVtdTreeIndex = prevLevelVtdTree.findIndex(
          ({ header }) => header === selectValues.at(-1),
        );
